<template>
    <div class="shirt-container relative">
        <svg class="relative px-2" version="1.1" id="Ebene_1" xmlns="http://www.w3.org/2000/svg"
            xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 520 580"
            style="enable-background:new 0 0 520 580;" xml:space="preserve">
	...
	c29.5,0.3,43.6-1.3,47.7,7.4" />
        ...
        <img v-if="designerStore.currentDesign.motiveObject" :src="designerStore.currentDesign.motiveObject.img"
            class="motive absolute" />
    </div>
</template>
...
function setShirtColor(shade1: String, shade2: String) {
    var elementCollection1 = document.getElementsByClassName("st0");
    var elementCollection2 = document.getElementsByClassName("st1");
    var elementCollection3 = document.getElementsByClassName("st2");

    Array.from(elementCollection1).forEach((element) => {
        element.style.fill = designerStore.currentDesign.colorObject?.color || "#787878";
    });
    Array.from(elementCollection2).forEach((element) => {
        element.style.fill = shade2;
    });
    Array.from(elementCollection3).forEach((element) => {
        element.style.fill = shade1;
    });
}

watch(designerStore.currentDesign, (newVal: IDesign) => {
    if (!newVal.colorObject) return;
    const shade1: String = chroma(newVal.colorObject.color).darken(1).hex()
    const shade2: String = chroma(newVal.colorObject.color).darken(2).hex()
    setShirtColor(shade1, shade2);
})
</script>